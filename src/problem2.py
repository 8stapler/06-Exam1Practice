...
Authors: David Mutchler, Vibha Alangar, Valerie Galluzzi, Mark Hays,
         Amanda Stouder, their colleagues and Russel Staples.
...
def problem2b(rect, n, delta, win):
    ...
    win.render()
    for k in range(n):
        rectangle = rg.Rectangle(rg.Point(rect.get_upper_left_corner().x-(delta*k), rect.get_upper_left_corner().y-(delta*k)), rg.Point(rect.get_lower_right_corner().x+(delta*k), rect.get_lower_right_corner().y+(delta*k)))
